 ...
 */
  Future<Database> _openConnection() async {
    ...
    return openDatabase(
      ...
      return db.execute('''CREATE TABLE collection(
  ...
  Future<void> addMovie(Movie movie) async {
    ...
    await db.insert('collection', movie.toMap(),
  ...
  Future<List<Movie>> getCollection() async {
    ...
    final List<Map<String, dynamic>> maps =
        await db.query('collection', orderBy: 'title');
    if (maps.isEmpty) {
      return <Movie>[];
    }
  ...
  Future<bool> inCollection(int id) async {
    ...
    return db.query('collection',
        where: 'id = ?', whereArgs: [id]).then((result) => result.isNotEmpty);
  }
}
